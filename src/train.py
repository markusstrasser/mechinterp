...
def train(config: TrainConfig, model: HookedTransformer, wandb_run=None):
    ...
    for step in range(config.steps):
        ...
        first_eval = step == config.eval_interval
        if is_eval:
            log_dict(log_data, with_keys=first_eval)

            # 1. Construct the context dictionary for this event.
            # This dict must conform to the ProbeContext TypedDict schema.
            ...
            # 2. Trigger all probes registered to the 'on_step' event.
            probe_metrics = trigger_probes('manual', step_ctx)

            # 3. Log everything to W&B.
            # We nest probe metrics for better organization in the dashboard.
            log_data = {
        ...
        if is_checkpoint:
            if wandb_run:
                ...
                artifact = wandb.Artifact(name=f"model-{wandb_run.id}", type="model",
                                          metadata={k: getattr(config, k) for k in config.__annotations__})

                checkpoint_dir = "checkpoints"
                os.makedirs(checkpoint_dir, exist_ok=True)
                model_filename = f"{config.d_model}+{config.d_ffn}+{config.n_layers}+checkpoint_step_{step}.pt"
